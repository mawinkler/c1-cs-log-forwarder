...
def collect_rt(c1_url, api_key, start_time, end_time):
    """
    ...
    while True:
        ...
        try:
            response = requests.get(url, headers=post_header, verify=True)
...
def collect_dc(c1_url, api_key, start_time, end_time):
    """
...
def cef_rt(event, facility, host, port):
    ...
    c.set_field(
        ...
        + " "
        + "proc.pname="
        + event.get("proc.pname", "<NA>")
        + " "
        + "container.id="
        + event["container.id"]
        + " "
        + "container.image.tag="
        + event["container.image.tag"]
        + " "
        + "container.image.repository="
        + event["container.image.repository"]
        + " "
        + "container.image.digest="
        + event["container.image.digest"],
    )
...
def cef_dc(event, facility, host, port):
    ...
    if len(event["reasons"]) == 0:
        reason = {"action": "<N/A>", "resources": [], "rule": "<N/A>", "type": "Allow"}
...
def cef_dc_reasons(event, reason, facility, host, port):
    ...
    c.set_field(
        ...
        + " "
        + "exceptions="
        + json.dumps(event.get("exceptions", []))
        + " "
        + "kind="
        + event["kind"]
        + " "
        ...
        + event["policyDefinitionName"]
        + " "
        + "policyName="
        + event.get("policyName", "<NA>")
        + " "
        + "reason.action="
        + reason["action"]
        + " "
        + "reason.resources="
        + json.dumps(reason["resources"])
        + " "
        + "reason.rule="
        + reason.get("rule", "<N/A>")
        + " "
...
def collect(c1_url, api_key, host, port, facility, interval, overlap):
    """
    ...
    while True:
        ...
        events += collect_dc(c1_url, api_key, start_time, end_time)

        _LOGGER.debug("Query runtime events")
        events += collect_rt(c1_url, api_key, start_time, end_time)
